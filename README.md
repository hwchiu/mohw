...
# 使用 .env 注入設定執行
docker run --rm --env-file .env hwchiu/mohw:prometheus-analyzer-latest \
  --node  worker-01 \
  --start "2026-03-10 14:00:00" \
  --end   "2026-03-10 15:00:00"
...
│  目標節點:    worker-01  (label: instance)           │
│  LLM:         http://test.com                       │
│  模式:        自動偵測  Workers: 10                  │
╰─────────────────────────────────────────────────────╯

測試 Prometheus 連線...  ✓
📡 探索節點 'worker-01' 的 metrics（/api/v1/series）...
🔍 發現 147 個 metrics，啟動 10 workers 並行掃描...
正在探測 model 能力...
