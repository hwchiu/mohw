...
本工具的核心概念是：**你指定節點與時間範圍，讓 LLM 代替你去探索 metrics，自主找出異常，推導根本原因**。

```
你提供：Prometheus endpoint + 節點名稱 + 發生異常的時間範圍
...
- **節點精準分析**：只掃描指定節點的 metrics（透過 `/api/v1/series` 一次發現），不受全局噪音干擾
- **並行掃描**：多 worker 同時查詢，百個 metrics 也能快速完成
...
`--node`（節點名稱）為**必填參數**，工具只分析此節點的 metrics。
...
TARGET_NODE=worker-01          # 也可以在 .env 設定節點
NODE_LABEL=instance            # node_exporter 環境用 instance；k8s 用 node
```
...
工具會自動：
1. 驗證節點名稱已提供（CLI `--node` 或 `.env` 的 `TARGET_NODE`）
2. 測試 Prometheus 連線
3. 用 `/api/v1/series` 一次找出該節點的所有 metrics
4. 並行掃描 metrics，偵測 z-score 異常
5. 探測 LLM 的 function calling 能力，選擇最適合的分析模式
6. 輸出根本原因分析報告
...
Options:
  ...
  -n, --node        TEXT     目標節點名稱（必填，或設定 .env 的 TARGET_NODE）
                             node_exporter 範例：worker-01 / 192.168.1.10:9100
                             k8s 範例：k8s-worker-node-1
      --node-label  TEXT     代表節點的 label 名稱（優先於 .env 的 NODE_LABEL）
                             node_exporter 通常是 instance；k8s 通常是 node
                             預設：instance
  ...
  -m, --model       TEXT     Model ID（優先於 .env 的 LLM_MODEL_ID）
      ...
      --workers     INT      並行掃描 worker 數（預設 10）
```

### 節點名稱怎麼查

```bash
# 用 curl 列出 Prometheus 有哪些 instance（node_exporter 環境）
curl -s 'http://prometheus.internal:9090/api/v1/label/instance/values' \
  | python3 -m json.tool

# k8s 環境查 node label
curl -s 'http://prometheus.internal:9090/api/v1/label/node/values' \
  | python3 -m json.tool
```
...
LLM 自主決定：
  → 呼叫 detect_anomalies()    找出節點的異常 metrics
...
程式先執行 detect_anomalies() 掃描節點全部 metrics
↓
...
```bash
# 強制使用靜態分析（最快啟動）
python __main__.py --node worker-01 ... --mode c

# 強制使用 Full Agentic
python __main__.py --node worker-01 ... --mode a
```
...
| 工具名稱 | 說明 |
|---------|------|
| `detect_anomalies` | 掃描節點所有 metrics，回傳 z-score 異常清單（依嚴重度排序） |
...
若 Prometheus 使用 HTTPS（內部自簽憑證或企業內部 CA），透過以下方式設定憑證驗證。

### 優先順序

```
--ssl-verify（CLI 參數）> PROMETHEUS_SSL_VERIFY（.env）> 預設（系統憑證）
```
...
| 設定值 | 說明 | 適用場景 |
|--------|------|---------|
| `true`（預設） | 自動載入作業系統信任鏈 | Prometheus 憑證已加入系統信任鏈 |
| `/path/to/ca.crt` | 指定自訂 CA bundle | 企業內部 CA / 自簽憑證 |
| `false` | 跳過憑證驗證 | 僅限測試，不建議正式使用 |

### 用法範例

**方式一：將內部 CA 憑證加入系統信任（推薦）**

```bash
# Linux（Debian / Ubuntu）
sudo cp /path/to/internal-ca.crt /usr/local/share/ca-certificates/
sudo update-ca-certificates

# Linux（RHEL / CentOS）
sudo cp /path/to/internal-ca.crt /etc/pki/ca-trust/source/anchors/
sudo update-ca-trust

# 設定後工具預設（ssl_verify=true）即可自動生效
python __main__.py --node worker-01 --prometheus https://prom.internal:9090 ...
```

**方式二：透過 `.env` 指定 CA bundle 路徑**

```bash
# .env
PROMETHEUS_URL=https://prom.internal:9090
PROMETHEUS_SSL_VERIFY=/etc/ssl/certs/internal-ca.crt
```

**方式三：透過 CLI 參數指定（臨時使用）**

```bash
python __main__.py \
  --prometheus   https://prom.internal:9090 \
  --node         worker-01 \
  --ssl-verify   /etc/ssl/certs/internal-ca.crt \
  --start "2026-03-10 14:00:00" \
  --end   "2026-03-10 15:00:00"
```

**方式四：Docker 容器（掛載 CA 憑證檔）**

```bash
docker run --rm \
  --env-file .env \
  -v /path/to/internal-ca.crt:/certs/ca.crt:ro \
  -e PROMETHEUS_SSL_VERIFY=/certs/ca.crt \
  hwchiu/mohw:prometheus-analyzer-latest \
  --node worker-01 \
  --start "2026-03-10 14:00:00" \
  --end   "2026-03-10 15:00:00"
```

> **注意：** 設定 `PROMETHEUS_SSL_VERIFY=false` 會讓連線無法驗證伺服器身份，僅適合開發環境。
